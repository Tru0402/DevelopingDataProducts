...
#Using MTCars dataset to predict MPG using the weight and cylinders of a vehicle

library(shinyapps)
library(shiny)
library(ggplot2)

# Creating server logic for shiny app

shinyServer(function(input, output) {
  
  mpgprediction <- reactive({
    mpg_model <- lm(mpg ~ cyl + wt, data = mtcars) #running the model
    x <- predict(mpg_model, newdata = data.frame(cyl = input$cyl, wt = input$wt)) #basing model on users selections
    return(x[1])    
  })
  output$mpgprediction <- renderText(sprintf("Prediction: %.1f MPG", mpgprediction())) #plotting mpg data
  output$graph_mpg <- renderPlot({ #creating visual predicted by model
    
    mtcars_data <- mtcars
    mtcars_data$data_source = "Motor Trend"
    selected_car <- data.frame(mpg = mpgprediction(), cyl = input$cyl, disp = 1, #creating car based on user's selections
